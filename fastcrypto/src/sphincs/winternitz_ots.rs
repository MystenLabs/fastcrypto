...
mod tests {
    ...
    #[test]
    fn test_convert_base_matches_wots_kat() {
        // From the WOTS+ sub-vector: message 0x10..0x1F, lg_w=4
        // chain_lengths: 1 0 1 1 1 2 1 3 ... 1 14 1 15
        let msg: Vec<u8> = (0x10u8..=0x1F).collect();
        let digits = convert_base(&msg, 4);
        assert_eq!(digits.len(), 32);
        assert_eq!(digits[0], 1); // 0x10 → [1, 0]
        assert_eq!(digits[1], 0);
        assert_eq!(digits[2], 1); // 0x11 → [1, 1]
        assert_eq!(digits[3], 1);
        assert_eq!(digits[30], 1); // 0x1F → [1, 15]
        assert_eq!(digits[31], 15);
    }

    #[test]
    fn test_wots_sign_cross_check_ref_c() {
        // Cross-check against Desktop/sphincs/kat/slh-dsa/wots_subvec.txt
        // pub_seed=0xA0..0xAF, sk_seed=0x50..0x5F, msg=0x10..0x1F
        // address: all zeros (layer=0, tree=0, type=WOTS_HASH, keypair=0)
        let pk_seed: Vec<u8> = (0xA0u8..=0xAF).collect();
        let sk_seed: Vec<u8> = (0x50u8..=0x5F).collect();
        let msg: Vec<u8> = (0x10u8..=0x1F).collect();
        let params = WotsParams::new(16, 4);
        let adrs = Adrs::new();

        let sig = wots_sign(&params, &sk_seed, &pk_seed, adrs, &msg);

        // Flatten and compare against ref C expected signature (560 bytes)
        let sig_flat: Vec<u8> = sig.iter().flatten().copied().collect();
        let expected = hex::decode(
            "60088aceb3ea7dbdcb47867c3b917416\
             0846d987db86cbc2259b7c2fd8f02c44\
             fab8a75c1b772287be5ee80244ba6b71\
             f5320a11fc5339574651fa3c6c08bc79\
             2ff0b43b169c2a6024b11d72b85e8302\
             06e9040c9f78306c6753de3f6d534ecf\
             55178fc72fa4fe79e72c23621677aa4d\
             8361d0307a5aa4bbb391e150a7cc4bfd\
             5c2843c7e96af51642808f018f733ff4\
             049668dba2df423660b481479031de27\
             c3374cdeb621965f6810335c9d901434\
             f107de11ef32e91e5984392953a60d30\
             d00c59d3c0e21bbfbaf9146f1c5c4096\
             5f9ee2ee124fe14d260fe63a16cf0992\
             7b07f701c313d7a2c223d7a035fb41dc\
             618414f54b8e57b5ded0e1ec273482b3\
             95c3291381256eb62072bb6cf6de8674\
             1d8462ec1324b124cd91fd5a0ce2e02d\
             aa7aecc63bade05f67398dbe300aa29f\
             11ad170947402283f586a5529f057432\
             bc39a5b59ed295576764907c7298a756\
             aa8efd8ff04f1adf1b7fc335557608a0\
             660e72c5472683afe3fdb1333ae57745\
             f7995adff062d24d25c8ed05aff90843\
             f5d1be6d13cd67ab2b1f417ebca75ad7\
             45c92e9dd67092808f8e0a8b39484dbe\
             d3df0c8fe018b50964cd0b147b63f767\
             293832b4b4076e9f821ebb0097182040\
             a24b23e785ae6f0b6d4ea9ba0d8dd43a\
             98b2c1c6e1f53c84e79fa073064e6217\
             539615d64507c8b32b421896b6a070dd\
             5e7b6423133b665806d420b726b74845\
             91b7e35bb5e4c21fcc041fc0225cacd5\
             04c37bb825528b56aa3f076a1c32aa8b\
             a049c31168fd0b66c81c572a80643c80",
        )
        .unwrap();
        assert_eq!(sig_flat.len(), 560);
        assert_eq!(
            sig_flat, expected,
            "wots_sign does not match ref C sub-vector"
        );
    }

    #[test]
    fn test_wots_keygen_cross_check_ref_c() {
        // Expected pk from wots_subvec.txt: a9401fe59245d3320379b07c20f55299
        let pk_seed: Vec<u8> = (0xA0u8..=0xAF).collect();
        let sk_seed: Vec<u8> = (0x50u8..=0x5F).collect();
        let params = WotsParams::new(16, 4);
        let adrs = Adrs::new();
        ...
        assert_eq!(hex::encode(&pk), "a9401fe59245d3320379b07c20f55299");
    }
