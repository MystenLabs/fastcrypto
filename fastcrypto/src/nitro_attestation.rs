...
//! Verification of AWS Nitro enclave attestation documents: COSE_Sign1 parsing,
//! attestation document validation and certificate chain verification against the
//! AWS Nitro root of trust, per
...
#[cfg(test)]
#[path = "tests/nitro_attestation_tests.rs"]
...
impl CoseSign1 {
    ...
    fn to_signed_message(&self) -> Result<Vec<u8>, NitroAttestationVerifyError> {
        ...
        ciborium::ser::into_writer(&value, &mut bytes).map_err(|_| {
            NitroAttestationVerifyError::InvalidCoseSign1("cannot serialize message".to_string())
        })?;
        Ok(bytes)
    }
}
