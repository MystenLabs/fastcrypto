...
//! A simple merkle tree implementation for Walrus & Sui.
...
pub trait MerkleAuth: Clone + std::fmt::Debug {
    ...
    fn verify_proof_with_unserialized_leaf<T: Serialize>(
        &self,
        root: &Node,
        leaf: &T,
        leaf_index: usize,
    ) -> FastCryptoResult<bool> {
        let bytes = to_bytes(leaf)?;
    ...
    L: Debug + Ord + Serialize,
{
    ...
    pub fn verify_proof(&self, root: &Node, target_leaf: &L) -> bool {
        ...
        if let Some((left_leaf, left_proof)) = &self.left_leaf {
            ...
            let is_valid_neighbor = self
                .is_valid_neighbor(left_leaf, left_proof, left_leaf_index, root)
                .expect("Failed to verify proof");
            if left_leaf >= target_leaf || !is_valid_neighbor {
                return false;
            }
        ...
        if let Some((right_leaf, right_proof)) = &self.right_leaf {
            let is_valid_neighbor = self
                .is_valid_neighbor(right_leaf, right_proof, right_leaf_index, root)
                .expect("Failed to verify proof");
            if right_leaf <= target_leaf || !is_valid_neighbor {
                return false;
            }
...
pub fn merkle_root<T, I>(iter: I) -> FastCryptoResult<[u8; DIGEST_LEN]>
where
    I: IntoIterator,
    ...
    T: HashFunction<DIGEST_LEN>,
{
    let tree = MerkleTree::<T>::build_from_unserialized(iter)?;
    Ok(tree.root().bytes())
}
    ...
    T: HashFunction<DIGEST_LEN>,
{
    ...
    /// Note: This is a simpler method than build_from_serialized because sometimes implementing AsRef<[u8]> makes the code
    /// more complex. On the other hand, we pay the cost of serializing the leaves multiple times (e.g., in the proof verification).
    ...
    pub fn compute_non_inclusion_proof<L: Debug + Ord + Clone>(
        &self,
        leaves: &[L],
        target_leaf: &L,
