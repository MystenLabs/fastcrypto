...
pub fn fr_p256_to_arkworks(scalar: &p256::Scalar) -> ark_secp256r1::Fr {
    ark_secp256r1::Fr::from_be_bytes_mod_order(scalar.to_bytes().as_slice())
}

pub fn fr_arkworks_to_p256(scalar: &ark_secp256r1::Fr) -> p256::Scalar {
...
pub fn fq_p256_to_arkworks(scalar: &p256::Scalar) -> ark_secp256r1::Fq {
    ark_secp256r1::Fq::from_be_bytes_mod_order(scalar.to_bytes().as_slice())
}

pub fn fq_arkworks_to_p256(scalar: &ark_secp256r1::Fq) -> p256::Scalar {
    let mut bytes = [0u8; 32];
    scalar
        .serialize_with_flags(&mut bytes[..], EmptyFlags)
        .unwrap();
...
fn get_coordinates(point: &p256::AffinePoint) -> (p256::Scalar, p256::Scalar) {
    ...
    (
        p256::Scalar::from_repr(point.x()).unwrap(),
        p256::Scalar::from_repr(FieldBytes::clone_from_slice(y.as_slice())).unwrap(),
    )
}

pub fn affine_pt_p256_to_arkworks(point: &p256::AffinePoint) -> ark_secp256r1::Affine {
    let (x, y) = get_coordinates(point);
    ark_secp256r1::Affine::new_unchecked(fq_p256_to_arkworks(&x), fq_p256_to_arkworks(&y))
}
...
pub fn affine_pt_arkworks_to_p256(point: &ark_secp256r1::Affine) -> p256::AffinePoint {
...
// Reduction
pub fn arkworks_fq_to_fr(scalar: &ark_secp256r1::Fq) -> ark_secp256r1::Fr {
    let mut bytes = [0u8; 32];
    scalar
        .serialize_with_flags(&mut bytes[..], EmptyFlags)
        .unwrap();
