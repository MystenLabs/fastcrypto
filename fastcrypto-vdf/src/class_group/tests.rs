...
fn test_qf_from_seed() {
    ...
    for _ in 0..10 {
        let qf = QuadraticForm::hash_to_group(&seed, &discriminant, 1);
    ...
    for _ in 0..10 {
        let qf = QuadraticForm::hash_to_group(&seed, &discriminant, 4);
        assert!(qf.is_reduced());
        assert_eq!(qf.discriminant(), discriminant);
        seed[0] += 1;
    }
...
fn qf_from_seed_sanity_tests() {
    ...
    let base_qf = QuadraticForm::hash_to_group(b"qf seed", &discriminant, 6);
    assert_eq!(base_qf.discriminant(), discriminant);

    // Same seed, same discriminant, same k
    let other_qf = QuadraticForm::hash_to_group(b"qf seed", &discriminant, 6);
    assert_eq!(base_qf, other_qf);

    // Smaller k
    let other_qf = QuadraticForm::hash_to_group(b"qf seed", &discriminant, 5);
    assert_ne!(base_qf, other_qf);

    // Larger k
    let other_qf = QuadraticForm::hash_to_group(b"qf seed", &discriminant, 7);
    assert_ne!(base_qf, other_qf);
    ...
    for _ in 0..1000 {
        ...
        let other_qf = QuadraticForm::hash_to_group(&seed, &discriminant, 6);
        assert_ne!(base_qf, other_qf);
    }

    let other_discriminant = Discriminant::from_seed(b"other_discriminant seed", 512).unwrap();
    // Same seed, same k, other discriminant
    let other_qf = QuadraticForm::hash_to_group(b"qf seed", &other_discriminant, 6);
    assert_ne!(base_qf, other_qf);
}
...
fn qf_from_seed_regression_tests() {
    ...
    let qf = QuadraticForm::hash_to_group(b"seed", &discriminant, 64);
    ...
    let qf = QuadraticForm::hash_to_group(b"seed", &discriminant, 32);
    ...
    let qf = QuadraticForm::hash_to_group(b"seed", &discriminant, 16);
    ...
    let qf = QuadraticForm::hash_to_group(b"seed", &discriminant, 8);
    ...
    let qf = QuadraticForm::hash_to_group(b"seed", &discriminant, 4);
