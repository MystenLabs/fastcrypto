...
use crate::{
    ...
    traits::{Cipher, EncryptionKey, ToFromBytes},
};
use digest::consts::*;
...
fn test_aes128ctr_encrypt_and_decrypt() {
    test_cipher::<16, 16, _, _>(|key| Aes128Ctr {
        key,
        iv: [0x24; 16],
    });
}
...
fn test_aes256ctr_encrypt_and_decrypt() {
    test_cipher::<32, 16, _, _>(|key| Aes256Ctr {
        key,
        iv: [0x24; 16],
    });
}
...
fn test_aes128gcm_encrypt_and_decrypt() {
    let nonce = GenericArray::<u8, digest::consts::U12>::from_slice(&[0x24; 12]);
    test_cipher_padded::<16, 12, _, _>(
        |key| Aes128Gcm {
            key,
            iv: nonce,
...
fn single_wycheproof_test_128<NonceSize: ArrayLength<u8>>(test: &Test) -> bool {
    let nonce: &GenericArray<u8, NonceSize> =
        GenericArray::<u8, NonceSize>::from_slice(&test.nonce);
    let cipher = Aes128Gcm {
        iv: nonce,
...
fn single_wycheproof_test<NonceSize: ArrayLength<u8>, C: Cipher>(test: &Test, cipher: C) -> bool {
    let mut ct_buffer = vec![0u8; test.pt.len() + 16];
    if cipher.encrypt(&test.pt, &mut ct_buffer).is_err() {
        return false;
    }
    ...
    if cipher.decrypt(&ct_buffer, &mut pt_buffer).is_err() {
        return false;
    }
