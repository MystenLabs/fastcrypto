...
#[test]
fn test_eval_and_commit() {
    // test zero
    let p = Poly::<RistrettoScalar>::zero();
    assert_eq!(p.eval(I10).value, RistrettoScalar::zero());
    // test consistency
    let p = Poly::<RistrettoScalar>::rand(5, &mut thread_rng());
    let e1 = p.eval(I10);
    let public_p = p.commit();
    let e2 = public_p.eval(I10);
    assert_eq!(e1.index, e2.index);
    assert_eq!(RistrettoPoint::generator() * e1.value, e2.value);
