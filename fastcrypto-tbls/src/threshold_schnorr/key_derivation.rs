...
pub(crate) fn derive_verifying_key_internal(vk: &G, index: u64) -> G {
    vk + G::generator() * compute_tweak(vk, index)
}

/// Derive a new verifying key from an existing one and a derivation index.
/// This will be a valid BIP-0340 Schnorr public key.
pub fn derive_verifying_key(vk: &G, index: u64) -> SchnorrPublicKey {
    SchnorrPublicKey::try_from(&derive_verifying_key_internal(vk, index)).expect("is never zero")
}
