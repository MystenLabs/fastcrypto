...
pub fn generate_partial_signatures<const BATCH_SIZE: usize>(
    ...
    verifying_key: &G,
    derivation_index: Option<(u64, &[u8])>,
    ...
    let verifying_key = if let Some((index, info)) = derivation_index {
        derive_verifying_key_internal(verifying_key, index, info)
    } else {
        *verifying_key
    };
...
pub fn aggregate_signatures(
    ...
    let verifying_key = if let Some((index, info)) = derivation_index {
        let tweak = compute_tweak(verifying_key, index, info);
        let derived_vk = derive_verifying_key_internal(verifying_key, index, info);
