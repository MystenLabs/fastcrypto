...
impl DkOutput {
    ...
    pub fn complete_dkg(
        ...
        // `try_reduce` would express this directly but is still nightly-only, so reduce over
        // `Result` items instead. The sanity check above guarantees a non-empty iterator.
        outputs
            .into_iter()
            .map(Ok)
