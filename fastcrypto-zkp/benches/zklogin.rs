...
mod zklogin_benches {
    ...
    fn verify_zk_login(c: &mut Criterion) {
        ...
        // Benchmark `calculate_all_inputs_hash` without cache so modulus-hash is computed every time.
        let eph_pubkey_clone = eph_pubkey.clone();
        let input_clone = input.clone();
        let modulus_clone = modulus.clone();
        ...
        // Benchmark the entire `verify_zk_login` function with cache hit.
        ...
        // Benchmark `verify_zk_login` without cache.
    ...
    criterion_group! {
        ...
        // TODO: Add a V2 benchmark after the updated proof and verifying key are available.
